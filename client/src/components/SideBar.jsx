    ...
    text-white ${selectedUser ? "max-md:hidden":''}`}>
     <div className="pb-5 ">
        ...
        <div className="bg-[#282142] rounded-full  flex items-center gap-2 py-3 px-4 mt-5 ">
            ...
            <input onChange={(e)=>setInput(e.target.value)} type="text" className="bg-transparent border-none outline-none text-white text-xs" placeholder="search..User"/>
     ...
     <div className="flex flex-col">
        {filteredUsers.map((user,index)=>(
            <div onClick={()=>{setSelectedUser(user); setUnseenMessages(prev=>({...prev ,[user._id]: 0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer
